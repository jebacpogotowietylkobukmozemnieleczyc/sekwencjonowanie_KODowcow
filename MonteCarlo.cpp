...
int MonteCarlo::successor(uint32_t nucleotide, unsigned offset) {
    ...
    if(successors.size()>2){
        for(uint32_t element :successors){
            ...
            maxRate = max(maxRate,newRate,[](const tuple<uint32_t ,unsigned >& tuple1,const tuple<uint32_t ,unsigned >& tuple2){
...
unsigned MonteCarlo::runForward(uint32_t nucleotide) {
    ...
    return runN(nucleotide, 2,50,[this](uint32_t nucleotide, unsigned offset){ return recursiveSuccessor(nucleotide,offset);}); //musi być mniejszy niż MAX_NEGATIVE

}

unsigned MonteCarlo::runBackward(uint32_t nucleotide){
    //todo //musi być mniejszy niż MAX_NEGATIVE
    return runN(nucleotide, 2,50,[this](uint32_t nucleotide, unsigned offset){ return recursivePredecessor(nucleotide,offset);}); //musi być mniejszy niż MAX_NEGATIVE
